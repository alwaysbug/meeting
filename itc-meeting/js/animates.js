...
//		$(this).css("border","1px solid #E6501E")
//	})
...
//		$option2.html("扫码");
		...
		var $login_content = $('<li class="login_btn_li"><button class="login_btn" type="button">登录</button><p class="login_control"><input type="checkbox" style="width: 14px;height: 14px;" />&nbsp;&nbsp;记住账号</li>');
	...
	function toResigter () {
		...
		var $intro = $('<h3>芮廷支持他们</h3><div class="logos"><img src="img/support_it.png"/></div><p>已有芮廷账号？<a href="javascript:;" class="login" onclick="toLogin()">登录</a></p>');
		$support.children().fadeOut();
		$support.children().remove();
		$intro.fadeIn();
		$support.append($intro);
...
function showTheWarning(code,data) {//0为错误  1为正确 2为警告
    ...
    switch (code){
        case 0:
            $warn_img.attr("src","img/wrong_tips.png");
            $warn_p.html(data.message || data.msg);
            break;
        case 1:
            $warn_img.attr("src","img/right_tips.png");
            $warn_p.html(data.message);
            break;
        case 2:
            $warn_img.attr("src","img/warning.png");
            $warn_p.html(data.message);
            break;
        default:
            break;
...
/*移动端*/
//移动端注册
$("#register_mobile").on("click",function () {

});
//移动端获取验证码
$("#get_verify_mobile").bind("click",getVerfiy);
function getVerfiy() {
    var param = {
        "mobile":$('#phone_mobile').val(),
        "template":"company_apply"
    };
    api.post("sms/send",param,sendTheCodeMobile);
}

function sendTheCodeMobile(data) {
	if (data.code === 0){
		var verify = $("#get_verify_mobile");
		verify.css({
		...
		var timer = setInterval(function () {
            ...
            verify.html("重发("+ num +"s)");
        },1000);
        $("#get_verify_mobile").unbind("click");
        setTimeout(function () {
            $("#get_verify_mobile").bind("click",getVerfiy);
            verify.html("重新获取验证码");
        },60000);
...
$("#register_mobile").on("click",function () {
    var param = {
        "company_name":$('#company_name_mobile').val(),
        "nickname":$('#name_mobile').val(),
        "mobile":$('#phone_mobile').val(),
        "email":$('#email_mobile').val(),
        "password":$('#password_mobile').val(),
        "code":$('#phone_verify_mobile').val(),
		"invite_code":$("#invite_code_mobile").val()
    };
    api.post("company/apply",param,function (data) {
		if (data.code === 0){
            swal({
                title: "注册成功",
            });
